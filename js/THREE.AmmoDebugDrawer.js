 ...
 */
 ...
 THREEx.AmmoDebugDrawer = function(scene, world, options) {
    this.scene = scene;
    this.world = world;
    options = options || {};

    this.debugDrawMode = options.debugDrawMode || THREEx.AmmoDebugConstants.DrawWireframe;
    var drawOnTop = this.debugDrawMode & THREEx.AmmoDebugConstants.DrawOnTop || false;
...
THREEx.AmmoDebugDrawer.prototype = function() {
    return this.debugDrawer;
};

THREEx.AmmoDebugDrawer.prototype.enable = function() {
    this.enabled = true;
    this.scene.add(this.mesh);
};

THREEx.AmmoDebugDrawer.prototype.disable = function() {
    this.enabled = false;
    this.scene.remove(this.mesh);
};

THREEx.AmmoDebugDrawer.prototype.update = function() {
    if (!this.enabled) {
        return;
    }
...
THREEx.AmmoDebugDrawer.prototype.drawLine = function(from, to, color) {
...
THREEx.AmmoDebugDrawer.prototype.drawContactPoint = function(pointOnB, normalOnB, distance, lifeTime, color) {
    const heap = Ammo.HEAPF32;
    const r = heap[(color + 0) / 4];
    const g = heap[(color + 4) / 4];
    const b = heap[(color + 8) / 4];
...
THREEx.AmmoDebugDrawer.prototype.reportErrorWarning = function(warningString) {
...
THREEx.AmmoDebugDrawer.prototype.draw3dText = function(location, textString) {
    //TODO
    console.warn("TODO: draw3dText");
};

THREEx.AmmoDebugDrawer.prototype.setDebugMode = function(debugMode) {
    this.debugDrawMode = debugMode;
};

THREEx.AmmoDebugDrawer.prototype.getDebugMode = function() {
